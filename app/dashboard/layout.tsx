    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {isImpersonating && (
        <div className="bg-amber-500 text-amber-950 px-4 py-2 flex items-center justify-center gap-4 text-sm font-medium">
          ...
          <span>
            Viewing as <strong>{tenant?.name}</strong> (impersonated by {impersonatedBy})
          </span>
          <Button
            variant="outline"
            size="sm"
              ...
              {isLoading ? (
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="ghost" className="w-full justify-between text-sidebar-foreground hover:bg-sidebar-accent h-auto py-2">
                      <div className="flex flex-col items-start gap-1">
                        <span className="truncate font-medium">{tenant?.name || 'Select Workspace'}</span>
                        {currentRole && (
                          <Badge variant="outline" className={cn("text-[10px] px-1.5 py-0", ROLE_COLORS[currentRole])}>
                  ...
                  <DropdownMenuContent align="start" className="w-64">
                    ...
                    {tenants.map((t) => (
                      ...
                      >
                        ...
                        <Badge variant="outline" className={cn("text-[10px] px-1.5 py-0 ml-2", ROLE_COLORS[t.role])}>
                          {ROLE_LABELS[t.role]}
                        </Badge>
                      </DropdownMenuItem>
                    ))}
                    ...
                    <DropdownMenuItem asChild>
                      <Link href="/dashboard/settings/new-workspace" className="flex items-center gap-2">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4">
                          <path d="M5 12h14"/>
                          <path d="M12 5v14"/>
                        </svg>
